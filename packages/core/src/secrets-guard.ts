...
const RUNTIME_SECRETS_ERROR =
  "Set BETTER_AUTH_SECRET and ENCRYPTION_KEY to long random strings before starting Troupe outside local development or tests.";

export function isDevSecretAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  if (env.TROUPE_ALLOW_DEV_SECRETS === "1") return true;
