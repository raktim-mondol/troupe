...
import type { ComputerAction, ComputerInput } from "@troupe/adapter-kit";
...
const SCREEN_REGISTRY = "/tmp/troupe/screen-assignments";
...
export function allocateExtraDisplayCommand(screenKey: string, leaseId?: string): string {
  ...
  return [
    ...
    'if [ -n "$owner" ] && [ -n "$current" ] && [ "$owner" != "$current" ] && [ "$incoming_fence" -le "$current_fence" ]; then printf \'TROUPE_SCREEN_INDEX=stale\\n\'; exit 75; fi',
    'tmp="$slot.$$"',
    'printf \'%s\\n%s\\n\' "$index" "$owner" >"$tmp"',
    'mv "$tmp" "$slot"',
    "printf 'TROUPE_SCREEN_INDEX=%s\\n' \"$index\"",
  ].join("\n");
}
...
export function releaseExtraDisplayCommand(screenKey: string, leaseId?: string): string {
  ...
  return [
    ...
    "flock 9",
    `slot="$dir/${key}.slot"`,
    "[ -f \"$slot\" ] || { printf 'TROUPE_SCREEN_RELEASE=missing\\n'; exit 0; }",
    "index=$(sed -n '1p' \"$slot\")",
    ...
    // biome-ignore lint/suspicious/noTemplateCurlyInString: POSIX parameter expansion in the remote shell script
    'case "$current" in *:*) rest=${current##*:}; case "$rest" in \'\'|*[!0-9]*) ;; *) current_fence=$rest; current_owner=${current%:*}; esac ;; esac',
    '[ -z "$owner" ] || [ "$current" = "$owner" ] || { [ "$incoming_owner" = "$current_owner" ] && [ "$incoming_fence" -ge "$current_fence" ]; } || { printf \'TROUPE_SCREEN_RELEASE=stale\\n\'; exit 0; }',
    'if [ "$index" -ne 0 ]; then display_number=$((index + 1)); view_port=$((6080 + index * 2)); control_port=$((6081 + index * 2)); view_vnc_port=$((5900 + index * 2)); control_vnc_port=$((5901 + index * 2)); pkill -f "Xvfb :$display_number -screen" || true; pkill -f "HOME=/tmp/fluxbox-home-$display_number DISPLAY=:$display_number fluxbox" || true; pkill -f -- "chromium-screen-$display_number" || true; pkill -f "^x11vnc .* -rfbport $view_vnc_port" || true; pkill -f "^x11vnc .* -rfbport $control_vnc_port" || true; pkill -f "^/usr/bin/python3 .*websockify.*$view_port" || true; pkill -f "novnc_proxy.*--listen $view_port" || true; pkill -f "^/usr/bin/python3 .*websockify.*$control_port" || true; pkill -f "novnc_proxy.*--listen $control_port" || true; rm -f "/tmp/.X$display_number-lock" "/tmp/.X11-unix/X$display_number" "/tmp/troupe/control-token-$display_number" "/tmp/troupe/view-password-$display_number" "/tmp/troupe-view-$display_number.vncpass" "/tmp/troupe-control-$display_number.vncpass" "/tmp/troupe/screen-$display_number.lock"; fi',
    'rm -f "$slot"',
    "printf 'TROUPE_SCREEN_RELEASE=%s\\n' \"$index\"",
  ].join("; ");
}

export function parseAllocatedExtraDisplay(output: string): number {
  const index = Number(output.match(/TROUPE_SCREEN_INDEX=(\d+)/)?.[1]);
...
export function parseReleasedExtraDisplay(output: string): number | undefined {
  const match = output.match(/TROUPE_SCREEN_RELEASE=(\d+)/);
...
export function ensureExtraDisplayCommand(
  ...
  const log = `/tmp/troupe/screen-${layout.displayNumber}`;
  ...
  const tokenFile = `/tmp/troupe/control-token-${layout.displayNumber}`;
  const passwordFile = `/tmp/troupe/view-password-${layout.displayNumber}`;
  const passwordAuthFile = `/tmp/troupe-view-${layout.displayNumber}.vncpass`;
  return [
    "set -eu",
    `mkdir -p /tmp/troupe ${fluxHome}/.fluxbox /tmp/.X11-unix ${profile}`,
    `exec 8>${shellQuote(`/tmp/troupe/screen-${layout.displayNumber}.lock`)}`,
    "flock 8",
    ...
    `if xdpyinfo -display ${layout.display} >/dev/null 2>&1 && (echo >/dev/tcp/127.0.0.1/${layout.viewVncPort}) >/dev/null 2>&1 && (echo >/dev/tcp/127.0.0.1/${layout.viewPort}) >/dev/null 2>&1; then printf 'TROUPE_SCREEN_PASSWORD=%s\n' "$view_password"; exit 0; fi`,
    ...
    `  exit 1`,
    `fi`,
    `for i in $(seq 1 50); do if (echo >/dev/tcp/127.0.0.1/${layout.viewVncPort}) >/dev/null 2>&1 && (echo >/dev/tcp/127.0.0.1/${layout.viewPort}) >/dev/null 2>&1; then printf 'TROUPE_SCREEN_PASSWORD=%s\n' "$view_password"; exit 0; fi; sleep 0.1; done`,
    "exit 1",
  ].join("\n");
}

export function parseExtraDisplayViewPassword(output: string): string {
  const password = output.match(/TROUPE_SCREEN_PASSWORD=([A-Za-z0-9_-]+)/)?.[1];
...
export function extraDisplayControlStartCommand(
  ...
  const passwordFile = `/tmp/troupe-control-${layout.displayNumber}.vncpass`;
  const tokenFile = `/tmp/troupe/control-token-${layout.displayNumber}`;
  const log = `/tmp/troupe/screen-${layout.displayNumber}`;
  return [
    "set -eu",
...
export function extraDisplayControlStopCommand(
  ...
  const stop = [
    ...
    `rm -f /tmp/troupe-control-${layout.displayNumber}.vncpass`,
    `rm -f /tmp/troupe/control-token-${layout.displayNumber}`,
  ].join("; ");
  if (!controlToken) return stop;
  return `[ -f /tmp/troupe/control-token-${layout.displayNumber} ] && [ "$(cat /tmp/troupe/control-token-${layout.displayNumber})" != ${shellQuote(controlToken)} ] || { ${stop}; }`;
}

export function observeExtraDisplayCommand(layout: ExtraDisplayLayout): string {
  const imagePath = `/tmp/troupe/observe-${layout.displayNumber}.png`;
  return [
    `DISPLAY=${layout.display} xdotool getmouselocation --shell 2>/tmp/troupe/cursor-${layout.displayNumber}.txt || true`,
    ...
    `printf '\\nCURSOR '`,
    `tr '\\n' ' ' </tmp/troupe/cursor-${layout.displayNumber}.txt 2>/dev/null || true`,
  ].join("; ");
}
