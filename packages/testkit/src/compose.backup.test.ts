...
function postgresUp() {
  try {
    execSync(`docker compose -f ${composeFile} exec -T postgres pg_isready -U troupe`, {
      stdio: "ignore",
      timeout: 8_000,
    });
    return true;
...
describeBackup("compose backup and restore", () => {
  it("dumps postgres and restores into a side database", () => {
    ...
    execSync(`${backupScript} ${stamp}`, {
      stdio: "pipe",
      timeout: 60_000,
      env: { ...process.env, TROUPE_BACKUP_SKIP_HOMES: "1" },
    });
    const dump = path.resolve("backups", stamp, "troupe.sql");
    ...
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U troupe -d postgres -c "DROP DATABASE IF EXISTS troupe_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U troupe -d postgres -c "CREATE DATABASE troupe_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U troupe -d troupe_restore_test`,
      {
        input: sql,
    ...
    const tables = execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U troupe -d troupe_restore_test -c "\\dt"`,
      { encoding: "utf8", timeout: 20_000 },
    );
    expect(tables).toMatch(/bots/);
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U troupe -d postgres -c "DROP DATABASE troupe_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
