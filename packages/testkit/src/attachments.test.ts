...
describeAttachments("chat attachments", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "troupe-attachments-"));
  ...
  it("uploads, sends with image/file, and rejects invalid attachments", async () => {
    const cookie = await signup(app, `attachments-${stamp}@troupe.test`, "Attachment User");
  ...
  it("attaches a workspace file into the thread", async () => {
    const cookie = await signup(app, `attach-thread-${stamp}@troupe.test`, "Attach Thread User");
    const bot = await rpc<{ id: string }>(app, cookie, "bots/create", {
      name: "Attacher",
      title: "Attacher",
