...
until docker compose -f "$ROOT/infra/compose/docker-compose.yml" exec -T postgres pg_isready -U troupe >/dev/null 2>&1; do
  sleep 1
done
docker compose -f "$ROOT/infra/compose/docker-compose.yml" exec -T postgres psql -U troupe -d troupe < "$SRC/troupe.sql"
