...
Same as the README quick start: `.env` from `.env.example`, Postgres via Compose, `pnpm sandbox:build`, `pnpm dev`, then [http://127.0.0.1:5173](http://127.0.0.1:5173). Electron: `pnpm --filter @troupe/desktop dev` while that stack is up.

## Docker Compose (single machine)

1. Copy `.env.example` to `.env` and set `BETTER_AUTH_SECRET` and `ENCRYPTION_KEY` to long random strings. Troupe refuses placeholder or missing secrets outside `development` / `test` (or when `TROUPE_ALLOW_DEV_SECRETS=1` is set).
...
Compose runs Postgres, the sandbox supervisor (Docker socket), API, worker, and a Vite preview of the web app. Bot computers are sibling containers (`troupe/computer:local`). The API process does not get an unrestricted Docker socket; the supervisor owns the lifecycle.
...
```env
TROUPE_LOCAL_MODELS=qwen3:4b,llama3.1:8b
TROUPE_LOCAL_MODELS_URL=http://127.0.0.1:11434/v1
TROUPE_LOCAL_CONTEXT_WINDOW=32768
TROUPE_LOCAL_MAX_TOKENS=4096
```

The loopback default is suitable when running Troupe from a source checkout. In Docker Compose,
...
leave Troupe through this URL. Leave `TROUPE_LOCAL_MODELS` blank to disable the provider.
...
the exact model id from that server, and an optional API key. By default Troupe only allows
loopback, RFC1918, and `host.docker.internal` targets. To permit public hostnames, set
`TROUPE_OPENAI_COMPAT_ALLOW_PUBLIC=1` in the deployment environment. Public hostnames must resolve
...
- **E2B** runs bot computers away from the Troupe host and is the recommended choice for public or multi-user production deployments. Troupe checkpoints the portable workspace and browser-profile directory to `DATA_DIR`; the E2B disk is a runtime cache, not the durable source of truth.
...
- **Box by ASCII** provides a managed Linux desktop through `BOX_API_KEY` and optionally `BOX_API_URL`. Troupe always creates or resumes boxes with `noEnv: true`, keeps the portable workspace under `/home/user/troupe-home`, and refreshes a two-hour TTL. A Box currently exposes one shared desktop, so concurrent Team bots can still use shell and files but only one can use graphical tools at a time.
...
2. Clone the repository on the VM and create a root `.env` with production-only values. At minimum set
   ...
   `TROUPE_HOST`, and the three public origins. Use URL-safe random values for database credentials.
3. Keep registration allowlisted while the service is private:

```env
NODE_ENV=production
TROUPE_HOST=app.example.com
# Optional operator-owned override, for example the Cloudflare allowlist file:
# CADDYFILE_PATH=/etc/troupe/Caddyfile.prod
BETTER_AUTH_URL=https://app.example.com
WEB_ORIGIN=https://app.example.com
API_URL=https://app.example.com
SIGNUPS_ENABLED=true
...
`/usr/local/sbin/troupe-backup` and enable the supplied `troupe-backup.timer`. It creates a verified
Postgres custom-format dump plus an application-data archive under `/var/backups/troupe`, with mode
...
Pull the new source, rebuild with `GIT_SHA=$(git rev-parse HEAD)`, run `pnpm --filter @troupe/db migrate` if you are not using the production Compose file, then restart API and worker. Product contracts stay compatible across cloud and self-hosted. `GET /health` should then report that commit as `"revision"`.

## What “Troupe Cloud” still needs
...
10. Turn on `SIGNUP_ALLOWLIST` until you want open registration. There is no Troupe-managed model billing in version 1 — users bring keys.

Expo / desktop installers are clients of that origin (`EXPO_PUBLIC_API_URL`, `TROUPE_WEB_URL`). They are not a Cloud control plane.
