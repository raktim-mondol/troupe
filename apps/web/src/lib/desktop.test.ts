...
import { type TroupeDesktop, windowChromeKind } from "./desktop.js";

function desktop(platform: string): TroupeDesktop {
  return {
    platform,
    window: {
...
describe("window chrome", () => {
  ...
  it("does not paint fake traffic lights into the browser shell or other pages", () => {
    ...
    const onboarding = readFileSync(path.join(root, "Onboarding.tsx"), "utf8");
    expect(shell).not.toContain("FF5F57");
    expect(onboarding).not.toContain("FF5F57");
  });
});
