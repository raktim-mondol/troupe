...
describe("normalizeApiBase", () => {
  it("trims, adds https, and keeps only the origin", () => {
    ...
    expect(normalizeApiBase("https://troupe.example.com:8443/api/")).toEqual({
      ok: true,
      url: "https://troupe.example.com:8443",
    });
...
describe("display and warnings", () => {
  it("shows host and non-default port", () => {
    expect(displayApiHost("https://troupe.example.com")).toBe("troupe.example.com");
  ...
  it("treats the compile-time default as not custom", () => {
    ...
    expect(usesCustomApiBase("https://troupe.example.com")).toBe(true);
  });
});

describe("probeApiBase", () => {
  it("accepts a Troupe /rpc/health response", async () => {
  ...
  it("rejects a host that is up but is not Troupe", async () => {
    const fetchImpl = vi.fn(
