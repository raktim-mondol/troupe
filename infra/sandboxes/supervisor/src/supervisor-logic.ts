...
export function stopExtraScreenCommand(index: number) {
  ...
  const profile = `/home/troupe/.browser-profiles/chromium-screen-${layout.displayNumber}`;
...
export function ensureScreenCommand(index: number) {
  ...
  return [
    ...
    `cp /etc/troupe/fluxbox/init ${fluxHome}/.fluxbox/init`,
    `cp /etc/troupe/fluxbox/apps ${fluxHome}/.fluxbox/apps 2>/dev/null || true`,
    `cp /etc/troupe/fluxbox/menu ${fluxHome}/.fluxbox/menu 2>/dev/null || true`,
    `HOME=${fluxHome} DISPLAY=${layout.display} fluxbox -rc ${fluxHome}/.fluxbox/init >${log}-fluxbox.log 2>&1 &`,
    `if [ -d /home/troupe/.browser-profiles/chromium ]; then cp -a /home/troupe/.browser-profiles/chromium/. ${profile}/; rm -f ${profile}/SingletonLock ${profile}/SingletonCookie ${profile}/SingletonSocket; fi`,
    `DISPLAY=${layout.display} HOME=/home/troupe troupe-browser --user-data-dir=${profile} >${log}-browser.log 2>&1 &`,
...
export function workspaceTarget(relative: string) {
  return relative ? path.posix.join("/home/troupe", relative) : "/home/troupe";
}
...
export function interactiveScreenCommand(
  ...
  const tokenFile =
    layout.displayNumber === 1
      ? "/tmp/troupe/control-token"
      : `/tmp/troupe/control-token-${layout.displayNumber}`;
  const stopProcesses =
    `pkill -f '^x11vnc .* -rfbport ${layout.controlVncPort}' || true; ` +
    `pkill -f '^/usr/bin/python3 .*websockify.*${layout.controlPort}' || true; ` +
    `rm -f ${tokenFile}`;
  ...
  return [
    ...
    `(x11vnc -display ${layout.display} -forever -shared -nopw -listen 127.0.0.1 -rfbport ${layout.controlVncPort} -xkb -ncache 0 >/tmp/troupe/x11vnc-control-${layout.displayNumber}.log 2>&1 &)`,
    `(websockify --web=/usr/share/novnc 0.0.0.0:${layout.controlPort} 127.0.0.1:${layout.controlVncPort} >/tmp/troupe/novnc-control-${layout.displayNumber}.log 2>&1 &)`,
